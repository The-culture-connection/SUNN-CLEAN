...
import type {
  CatalogItem, InvoicingSettings, LineItem, PropertyType, QuoteResult,
  SchedulingSettings, Surcharge, Millis,
} from './types.js';
import { estimateDuration } from './scheduling.js';
...
export interface PriceInput {
  ...
  timezone: string;
  /** UTC millis of the service start; enables time-dependent surcharges */
  startAt?: Millis;
  /** Extra line items (admin adjustments). Re-runs subtotal -> tax -> total. */
...
export function surchargeApplies(
  s: Surcharge,
  opts: { startAt?: Millis; timezone: string; floors: number; hasElevator: boolean },
): boolean {
  ...
  switch (s.trigger) {
    ...
    case 'after_hours': {
      ...
      const dt = DateTime.fromMillis(startAt, { zone: timezone });
    ...
    case 'weekend': {
      if (startAt == null) return false;
      const wd = DateTime.fromMillis(startAt, { zone: timezone }).weekday; // 6=Sat 7=Sun
      return wd === 6 || wd === 7;
    }
...
export function computeQuote(input: PriceInput): QuoteResult {
  const {
    service, propertyType, squareFeet, floors, hasElevator, addOns,
    surcharges, scheduling, invoicing, timezone, startAt, extraLineItems = [],
  } = input;

  const emptySnapshot: QuoteResult['rateSnapshot'] = {
  ...
  const fail = (reason: QuoteResult['reason'], durationMinutes = 0): QuoteResult => ({
  ...
  if (!service || service.kind !== 'service' || !service.active) return fail('invalid_service');
  if (!(squareFeet > 0)) return fail('invalid_input');
  ...
  for (const a of addOns) {
    if (!a.item || a.item.kind !== 'addon' || !a.item.active) continue;
  ...
  for (const s of surcharges) {
    if (!surchargeApplies(s, { startAt, timezone, floors, hasElevator })) continue;
  ...
  /* --- adjustments (admin) ---------------------------------------- */
  for (const e of extraLineItems) lineItems.push(e);
 ...
 */
export function retotal(lineItems: LineItem[], taxRate: number): {
