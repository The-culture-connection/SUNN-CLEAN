...
import type { Crew, PropertyType, Settings, Surcharge, Weekday, DayHours } from './types.js';

export const MISSION_STATEMENT =
  'At SUNN Clean LLC, our mission is to ensure cleanliness, sanitation, ' +
...
const weekday = (start: number, end: number): DayHours => ({ enabled: true, start, end });
const closed: DayHours = { enabled: false, start: 0, end: 0 };

export function defaultCrewHours(): Record<Weekday, DayHours> {
  return {
    sun: closed,
...
export function defaultCrew(index = 0): Omit<Crew, 'id'> {
  return {
    name: `Crew ${String.fromCharCode(65 + index)}`,
    color: CREW_COLORS[index % CREW_COLORS.length],
    active: true,
    headcount: 2,
    priority: index + 1,
 ...
 */
export const DEFAULT_PROPERTY_TYPES: Omit<PropertyType, 'id'>[] = [
...
export function defaultSettings(timezone = 'America/New_York'): Settings {
  return {
    business: {
    ...
    content: {
      ...
      heroSubhead:
        'Commercial cleaning you can book online in minutes — offices, medical ' +
